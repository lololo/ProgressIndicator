...
public extension ProgressIndicator {
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	func show(_ status: String? = nil, interaction: Bool = true) {

		DispatchQueue.main.async {
            self.setup(status: status, hide: false, interaction: interaction)
		}
	}

	// MARK: -
	//---------------------------------------------------------------------------------------------------------------------------------------------
	func show(_ status: String? = nil, icon: AlertIcon, interaction: Bool = true) {

		let image = icon.image?.withTintColor(self.colorAnimation, renderingMode: .alwaysOriginal)

		DispatchQueue.main.async {
            self.setup(status: status, staticImage: image, hide: true, interaction: interaction)
		}
	}

	//---------------------------------------------------------------------------------------------------------------------------------------------
	func show(_ status: String? = nil, icon animatedIcon: AnimatedIcon, interaction: Bool = true) {

		DispatchQueue.main.async {
            self.setup(status: status, animatedIcon: animatedIcon, hide: true, interaction: interaction)
		}
	}

	// MARK: -
	//---------------------------------------------------------------------------------------------------------------------------------------------
	func showSuccess(_ status: String? = nil, image: UIImage? = nil, interaction: Bool = true) {

		DispatchQueue.main.async {
            self.setup(status: status, staticImage: image ?? self.imageSuccess, hide: true, interaction: interaction)
		}
	}

	//---------------------------------------------------------------------------------------------------------------------------------------------
	func showError(_ status: String? = nil, image: UIImage? = nil, interaction: Bool = true) {

		DispatchQueue.main.async {
            self.setup(status: status, staticImage: image ?? self.imageError, hide: true, interaction: interaction)
		}
	}

	// MARK: -
	//---------------------------------------------------------------------------------------------------------------------------------------------
	func showSucceed(_ status: String? = nil, interaction: Bool = true) {

		DispatchQueue.main.async {
            self.setup(status: status, animatedIcon: .succeed, hide: true, interaction: interaction)
		}
	}

	//---------------------------------------------------------------------------------------------------------------------------------------------
	func showFailed(_ status: String? = nil, interaction: Bool = true) {

		DispatchQueue.main.async {
            self.setup(status: status, animatedIcon: .failed, hide: true, interaction: interaction)
		}
	}

	//---------------------------------------------------------------------------------------------------------------------------------------------
	func showAdded(_ status: String? = nil, interaction: Bool = true) {

		DispatchQueue.main.async {
            self.setup(status: status, animatedIcon: .added, hide: true, interaction: interaction)
		}
	}

	// MARK: -
	//---------------------------------------------------------------------------------------------------------------------------------------------
	func showProgress(_ progress: CGFloat, interaction: Bool = false) {

		DispatchQueue.main.async {
            self.setup(progress: progress, hide: false, interaction: interaction)
		}
	}

	//---------------------------------------------------------------------------------------------------------------------------------------------
    func showProgress(_ status: String?, _ progress: CGFloat, interaction: Bool = false) {

		DispatchQueue.main.async {
            self.setup(status: status, progress: progress, hide: false, interaction: interaction)
		}
	}
}
...
public class ProgressIndicator: UIView {
	...
	static let shared: ProgressIndicator = {
		let instance = ProgressIndicator()
		return instance
	} ()
